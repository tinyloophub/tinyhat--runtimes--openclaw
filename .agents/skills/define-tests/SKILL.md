...
| Runtime code/config, bootstrap, install, or dev image | Above plus `bash -n bootstrap.sh dev/entrypoint.sh`; `python -m unittest tests.test_supervisor -v`; `docker build -f dev/Dockerfile -t tinyhat-openclaw-runtime:<topic> .`; focused tests for the changed module |
